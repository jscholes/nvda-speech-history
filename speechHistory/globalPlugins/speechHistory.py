...
def mySpeak(sequence, *args, **kwargs):
	global data
	text = u''.join([x for x in sequence if isinstance(x, basestring)]).strip() if sys.version_info.major == 2 else u' '.join([x for x in sequence if isinstance(x, str)]).strip()
	if text:
		data = text
